...
### 2. **Node Server** (Alternative Backend - server-node branch)
- **Frameworks/Technologies:** Node.js, Express, Socket.IO, JWT
- **Folder Contents:**
  - `config/` 
    - `configs.js`: Loads and exports environment-based configuration values (e.g., DB credentials).
  - `logs/`: Stores log output files generated by the logger utility. (Note: This folder must exists)
  - `middlewares/`: Express and Socket.IO middleware
    - `auth.js`: Middleware for verifying JWT tokens in HTTP requests.
    - `sockets.js`: Middleware for authenticating and handling Socket.IO events.
  - `models/`: Sequelize models
    - `index.js`: Initializes Sequelize and defines relationships.
    - `LoginAttempt.js`: Tracks user login attempts.
    - `Message.js`: Defines message schema for chat storage.
    - `User.js`: Defines the user schema and authentication logic.
  - `routes/`: REST API endpoints
    - `auth.js`: Routes for user signup and login.
    - `messages.js`: Routes for fetching and managing chat messages.
    - `socket.js`: Socket-related REST endpoints.
    - `users.js`: Routes for fetching user data.
  - `seeders/`: (Optional) Seeder scripts for populating initial database data.
  - `utils/`
    - `logger.js`: Custom logger setup for structured and file-based logging.
  - `validation/`:
    - `index.s`
    - `validators.js`: Joi custom validators for request payloads.
  - `.env`: Environment file containing sensitive variables such as DB credentials and JWT secret.
  - `.gitignore`: Git ignored files and folders.
  - `package-lock.json`: Lock file for NPM dependency versions.
  - `package.json`: Defines the Node project metadata and dependencies.
  - `main.js`: Entry point for running the Node.js server. Uses the following environment variables:
